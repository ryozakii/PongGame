...
const PongGame: React.FC<PongGameProps> = ({
  ...
  useEffect(() => {
    ...
    if (savedState) {
      ...
      score1.current = state.score1;
      score2.current = state.score2;
      start.current = state.start;
      setGameOver(state.gameOver);
      setWinner(state.winner);

      if (state.leftPaddle) {
        leftPaddle.current = createPaddle(
          state.leftPaddle.x,
          state.leftPaddle.y
        );
      }
      if (state.rightPaddle) {
        rightPaddle.current = createPaddle(
          state.rightPaddle.x,
          state.rightPaddle.y
        );
      }
      ...
      if (state.ball) {
        ball.current.x = state.ball.x;
        ball.current.y = state.ball.y;
        ball.current.vx = state.ball.vx;
        ball.current.vy = state.ball.vy;
      }
    }
  }, []);
  ...
  useEffect(() => {
    const saveGameState = () => {
      const gameState = {
        ...
        winner,
        leftPaddle: {
          x: leftPaddle.current.x,
          y: leftPaddle.current.y,
        },
        rightPaddle: {
          x: rightPaddle.current.x,
          y: rightPaddle.current.y,
        },
        ball: {
          x: ball.current.x,
          y: ball.current.y,
  ...
  const handleKeyDown = (e: KeyboardEvent) => {
    ...
    if (gameOver && e.key.toLowerCase() === "m") {
      onReturnToMenu();
    }
  };
