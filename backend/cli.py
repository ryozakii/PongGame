...
class CursesPong:
    ...
    def draw_game_state(self):
        ...
        user2 = self.player1.get("username", "Player 2")
        ...
        while True:
            try:
                ...
                if key == ord('q'):
                    ...
                    return False
                elif key == ord('w') and self.last_move != 'w':
                    message = {'move': 'up', 'type': 'paddle_move'}
                    self.last_move = 'w'
                elif key == ord('s') and self.last_move != 's':
    ...
    async def connect_to_websocket(self):
        url = f"wss://{os.getenv('NEXT_PUBLIC_WS_BASE_URL')}/ws/ping_pongw/"
    ...
    def login(self):
        ...
        # Attempt login
        try:
            url = f"http://{os.getenv('NEXT_PUBLIC_WS_BASE_URL')}/api/login"
